...
export default function Drawer() {
  ...
  const [toggleEcommerce, setToggleEcommerce] = useState(false);
  let ecomMenu = false;
  return (
    <>
      {/* <button onClick={() => setOpen(!open)}>-translate-x-full</button> */}
      <div
        id="drawer-navigation"
        className={`h-[calc(100svh-4.5rem)] p-4  bg-white w-64 shadow-md transition-transform  overflow-y-auto
         ${drawer ? "absolute -translate-x-full" : "relative "} 
         `}
      >
        ...
        <div className="py-4 overflow-y-auto">
          <ul className="space-y-2 font-medium">
            ...
            <li>
              <button
                onClick={() => setToggleEcommerce(!toggleEcommerce)}
                type="button"
                className={`flex items-center w-full p-2 text-base text-gray-900  rounded-lg group hover:bg-gray-100 transition duration-75 bg-none focus:bg-red-500 `}
              ...
              <ul
                id="dropdown-example"
                className={`py-2 space-y-2 ${toggleEcommerce && "hidden"}`}
              >
                <li>
                  <a
                    href="#"
